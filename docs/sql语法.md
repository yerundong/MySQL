...
**关键字：**ALTER TABLE、ADD COLUMN

**语法：**

```sql
ALTER TABLE 表名 ADD [COLUMN] 新列名 新类型[(长度)]; -- [COLUMN]可省略
```
...
- 删除单表记录：
  ...
  ```sql
  DELETE [表名/别名] FROM 表名 AS 别名 WHERE 筛选条件; -- 删除单表记录时，[表名/别名]可以省略
...
3. 建表时，类型后若不写长度，则会有默认长度；长度不是限制数值范围的（数值范围只由类型决定），而是显示的长度，需要追加 ZEROFILL 关键字才能体现
